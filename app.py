...
metrics = [
    ...
    ("RSI", f"{rsi_val:.1f}", "Overbought" if rsi_val > 70 else ("Oversold" if rsi_val < 30 else "Neutral")),
    ("Avg Vol", f"{vol_avg/1e6:.2f}M", None),
...
with tab3:
    ...
    st.caption(f"Exports last {len(df)} trading days for {symbol}")

# ─── Footer ───────────────────────────────────────────────────────────────────
st.markdown(
    '<p class="footer">⚠️ For educational purposes only · Not financial advice · Data via Yahoo Finance</p>',
    unsafe_allow_html=True
)